...
def loginPage(error = False):
	global uname_entry,label,uname_label,login_button,passwd_label,passwd_entry,tag
	...
	passwd_label=tk.Label(window, text="Password")
	...
	if error:
		...
		label2 = tk.Label(window, text=error)
		label2.grid(row=3,column=1)
		label2.after(2500,invalidUsername)
		error = None
	login_button.bind("<Button-1>",destroyloginPage)
	passwd_entry.bind("<Return>",destroyloginPage)

def destroyloginPage(event):
	global window,USERNAME,uname_entry,tag,label,uname_label,login_button,passwd_entry
	print('dgfdhdhd')
	if(tag==1):
		USERNAME=uname_entry.get()
		passwd=passwd_entry.get()
		print(USERNAME+":"+passwd)
		data = pickle.dumps({'type': 'login', 'author': USERNAME, 'data':passwd})
		transmitbytes(s, data)
		data = pickle.loads(receivebytes(s))
		print(data)
		if data['type'] == 'error':
			#label.grid_forget()
			#uname_label.grid_forget()
...
def connectedPage():
	...
	#--
	label3 =tk.Label(window, text='Change Password', font=LARGE_FONT).grid(row=6)
	change_passwd_label=tk.Label(window, text="Enter new password")
	change_passwd_label.grid(row=7,column=0)
	change_passwd_entry=tk.Entry(window,show='*')
	change_passwd_entry.grid(row=7,column=1)
	change_passwd_button=tk.Button(window, text='Change password')
	change_passwd_button.grid(row=7,column=2)


	def leaveGroup(event):

		left_group_label=tk.Label(window, text="Left "+leave_group_entry.get()+" group")
		left_group_label.grid(row=4,column=1)
		data = pickle.dumps({'type': 'leavegroup', 'data': leave_group_entry.get(), 'author': USERNAME})
		leave_group_entry.delete(0,tk.END)
		transmitbytes(s, data)
		def deleteLabel():
			left_group_label.grid_forget()

		left_group_label.after(2500,deleteLabel)

	def changePasswd(event):

		passwd_changed_label=tk.Label(window, text="Password changed !")
		passwd_changed_label.grid(row=6,column=1)
		data = pickle.dumps({'type': 'changepasswd', 'data': change_passwd_entry.get(), 'author': USERNAME})
		transmitbytes(s, data)
		change_passwd_entry.delete(0,tk.END)
		def deleteLabel():
			passwd_changed_label.grid_forget()

		passwd_changed_label.after(2500,deleteLabel)



	leave_group_button.bind("<Button-1>",leaveGroup)
	leave_group_entry.bind("<Return>",leaveGroup)
	change_passwd_button.bind("<Button-1>",changePasswd)
	change_passwd_entry.bind("<Return>",changePasswd)
