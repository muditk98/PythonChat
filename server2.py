...
def process_message(sock, data):
	if data['type'] == 'login':
		...
		if not row:
			...
			hashed_passwd = get_hashed_password(passwd)
			userCollection.insert_one({'name': name, 'hash': hashed_passwd})
			# transmitbytes(sock, pickle.dumps({'type': 'text', 'data': 'New user registered', 'author': 'server'}))
		else:
			hashed_passwd = row['hash']
			if not check_password(passwd, hashed_passwd):
				transmitbytes(sock, pickle.dumps({'type': 'error', 'data': 'Invalid password', 'author': 'server'}))
				return
		if name not in username_sock_table:
			...
			transmitbytes(sock, pickle.dumps({'type': 'error', 'data': 'User already logged in', 'author': 'server'}))
	elif username_sock_table[data['author']] != sock:
		return
	elif data['type'] == 'changepasswd':
		name = data['author']
		passwd = data['data']
		hashed_passwd = get_hashed_password(passwd)
		userCollection.update_one({'name': name}, {'$set': {'hash': hashed_passwd}})
		...
		if group_row:
			...
			print('Delete leave count:', count.deleted_count)
		else:
